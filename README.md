...
What it does
A producer describes the workflow in plain English.
The producer can attach a script or production PDF.
Vertex AI reads the PDF natively and extracts grounded characters, scenes, props, locations, and rights/clearance mentions.
The producer reviews the extracted facts.
...
Backlot generates and supervises the result.
A successfully built tool can be published as a standalone path under /tools/<slug>/.
Agent architecture
